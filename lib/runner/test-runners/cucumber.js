...
class CucumberSuite extends TestSuite {
  ...
  getCliArgvForCucumber() {
    ...
    if (process.env.NIGHTWATCH_RETRY) {
      retryOptions = '--retry=' + process.env.NIGHTWATCH_RETRY;
    }
    ...
    ].concat(feature_path, parallelArgs, specs, additionalOptions, retryOptions, extraParams);
  }
